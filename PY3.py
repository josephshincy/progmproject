...
# define a function that takes in the accepted ticker for the required stock and returns that stock's daily adjusted
# information from AlphaVantage
def getalpha(stock):
    alpha = pd.read_csv(
        'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=' + stock + '&apikey=' + key +
        '&datatype=csv')
    return alpha


# define a function that takes in the accepted ticker for the required stock and returns that stock's daily adjusted
# information from yfinance
def getyahoo(stock):
